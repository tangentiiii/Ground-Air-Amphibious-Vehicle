...
#define SLEW_STEP 0.04f //变化率限制，速度每一轮循环最多变化多少，避免电机极大的加速度
#define MIN_EFFECTIVE_CMD 0.12f //最小的有效占空比（占空比太小有可能推不动电机）

// HC-SR04 超声波避障：PB8=Trig，PB9=Echo。PB9按你的硬件确认可承受5V，所以Echo可直接接PB9。
...
#define US_STATE_WAIT_RISE 1
#define US_STATE_WAIT_FALL 2
#define US_STATE_DONE      3
...
volatile uint32_t us_timeout_count = 0;      //debug：测距超时次数
static uint16_t us_trigger_time = 0;         //触发本次测距的时间，单位us计数
static uint32_t us_last_sample_ms = 0;       //上一次触发测距的毫秒时间
...
int main(void)
{
  ...
  while (1)
  {
    ...
    //HC-SR04非阻塞测距任务：只负责触发测距、处理完成/超时，不会等待Echo，因此不会卡住电机控制
    hcsr04_task(now_ms);
    ...
    //避障逻辑：前方过近时，只禁止继续前进；后退和原地转向仍然允许，用于脱困
    if ((obstacle_blocked != 0) && (throttle > FORWARD_CMD_THRESHOLD))
    {
      throttle = 0.0f;
    }
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOB, GPIO_PIN_8|GPIO_PIN_10|GPIO_PIN_11|GPIO_PIN_12|GPIO_PIN_13
  ...
  /*Configure GPIO pin : PB8，HC-SR04 Trig */
  ...
  /*Configure GPIO pin : PB9，HC-SR04 Echo，使用外部中断，上升沿/下降沿都触发 */
...
static void delay_us(uint16_t us)
{
  ...
  {
    //只用于产生10us Trig脉冲。这里最多阻塞10us，不会影响电机控制。
  }
}

static void obstacle_update_from_distance(float distance_cm)
{
  if ((distance_cm > 0.0f) && (distance_cm < OBSTACLE_STOP_CM))
  {
    obstacle_blocked = 1;
  }
  else if ((distance_cm < 0.0f) || (distance_cm > OBSTACLE_RELEASE_CM))
  {
    obstacle_blocked = 0;
  }
}
...
static void hcsr04_start_measurement(void) //触发一次10us的Trig信号
{
  ...
  __disable_irq();
  us_state = US_STATE_WAIT_RISE;
  us_echo_width = 0;
  us_trigger_time = (uint16_t)__HAL_TIM_GET_COUNTER(&htim2);
  __enable_irq();

  //HC-SR04要求Trig至少10us高电平。这里仅阻塞10us，不再等待Echo。
...
static void hcsr04_echo_exti_callback(void)
{
  ...
  if (HAL_GPIO_ReadPin(US_ECHO_PORT, US_ECHO_PIN) == GPIO_PIN_SET)
  {
    //Echo上升沿：记录高电平开始时间
    if (us_state == US_STATE_WAIT_RISE)
    {
      us_echo_start = now;
      us_state = US_STATE_WAIT_FALL;
    }
  }
  else
  {
    //Echo下降沿：记录高电平宽度，测距完成
    if (us_state == US_STATE_WAIT_FALL)
    {
      us_echo_width = (uint16_t)(now - us_echo_start);
      us_state = US_STATE_DONE;
    }
  }
}
